...
public class MagicItemNameResolver implements ItemNameResolver {
	...
	private MaterialData resolveBlockData(Material type, String sdata) {
		if (type == WrappedMaterial.getWrapper().getLog() || type == WrappedMaterial.getWrapper().getSapling() || type == WrappedMaterial.getWrapper().getWood()) {
			return getTree(sdata);
		} else if (type == WrappedMaterial.getWrapper().getLeaves()) {
			return getLeaves(sdata);
		} else if (type == WrappedMaterial.getWrapper().getWool()) {
	...
	private MaterialData resolveItemData(Material type, String sdata) {
		if (type == WrappedMaterial.getWrapper().getInkSack()) {
			return getDye(sdata);
		} else {
			return null;
		}
	...
	private MagicMaterial resolveUnknown(String stype, String sdata) {
		try {
			Material material = Material.getMaterial(stype);
			if (sdata.equals("*")) {
