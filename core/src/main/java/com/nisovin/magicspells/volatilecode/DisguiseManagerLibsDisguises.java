...
public class DisguiseManagerLibsDisguises implements Listener, IDisguiseManager {
	...
	protected Map<String, Disguise> disguises = new ConcurrentHashMap<>();
	protected Map<String, me.libraryaddict.disguise.disguisetypes.Disguise> libsDisguises = new ConcurrentHashMap<>();
	protected Map<Integer, Disguise> disguisedEntityIds = new ConcurrentHashMap<>();
	...
	public void removeDisguise(Player player, boolean sendPlayerPackets, boolean delaySpawnPacket) {
		Disguise disguise = this.disguises.get(player.getName().toLowerCase());
	...
	private me.libraryaddict.disguise.disguisetypes.Disguise getDisguiseLibDisguise(Disguise dis) {
		...
		if (disType.isMob()) {
			...
			if (w instanceof SkeletonWatcher) {
				...
				case 0: // No armor, already null
					break;
				case 1: // Iron armor
					horseArmor = new ItemStack(WrappedMaterial.getWrapper().getIronBarding(), 1);
					break;
				case 2:
					horseArmor = new ItemStack(WrappedMaterial.getWrapper().getGoldBarding(), 1);
					break;
				case 3:
					horseArmor = new ItemStack(WrappedMaterial.getWrapper().getDiamondBarding(), 1);
					break;
				}
