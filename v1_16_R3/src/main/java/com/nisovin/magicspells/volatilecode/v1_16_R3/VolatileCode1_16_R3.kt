...
class VolatileCode1_16_R3: VolatileCodeHandle {
    ...
    private val fireworks = java.util.ArrayList<EntityFireworks>()
    ...
    private fun ItemStack.asCraftCopy(): CraftItemStack {
    ...
    override fun playParticleEffect(location: Location, name: String, spreadX: Float, spreadY: Float, spreadZ: Float, speed: Float, count: Int, radius: Int, yOffset: Float) {
        location.world!!.spawnParticle(ParticleUtil.ParticleEffect.getParticle(name), location.add(0.0, yOffset.toDouble(), 0.0), count, spreadX.toDouble(), spreadY.toDouble(), spreadZ.toDouble(), speed)
    }
    ...
    override fun sendActionBarMessage(player: Player, message: String) {
        val conn = (player as CraftPlayer).handle.playerConnection
        val packet = PacketPlayOutChat(ChatComponentText(message), ChatMessageType.GAME_INFO,player.uniqueId)
        conn.sendPacket(packet)
    }
